...
const ShareScoreCard = ({ hotelName, scoreCardElementId }: ShareScoreCardProps) => {
    ...
    const handleShareViaEmail = async () => {
        ...
        try {
            ...
            // Upload PDF directly to storage from client (avoids memory limit in edge function)
            ...
            // Send only the URL to the edge function (lightweight call)
            ...
            toast({ title: 'Report sent!', description: `The score card report has been emailed to ${recipientEmail.trim()}.` });
            setShowEmailDialog(false);
            setRecipientEmail('');
        } catch (err) {
            console.error('Share error:', err);
            toast({ title: 'Share failed', description: 'Could not send the report email. Please try downloading the PDF instead.', variant: 'destructive' });
        } finally {
            setIsSharing(false);
        }
    };
