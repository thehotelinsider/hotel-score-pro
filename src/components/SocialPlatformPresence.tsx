...
/** Returns the account-found message based on accountCount / noDedicatedAccount flags */
const getAccountMessage = (platform: SocialPlatformMetrics): string => {
  if (platform.noDedicatedAccount || platform.accountCount === 0) {
    return 'No dedicated account was found';
  }
  if (typeof platform.accountCount === 'number' && platform.accountCount > 1) {
    return 'Several accounts have been found';
  }
  return '1 account has been found';
};

/** Derives whether the platform is considered active from accountCount first, then status */
const isActive = (platform: SocialPlatformMetrics): boolean => {
  // If an account was found (accountCount > 0), treat as active regardless of status field
  if (typeof platform.accountCount === 'number' && platform.accountCount > 0) return true;
  // Otherwise fall back to noDedicatedAccount + status
  return !platform.noDedicatedAccount && platform.status !== 'inactive';
};
...
const SocialPlatformItem = ({ platform }: { platform: SocialPlatformMetrics }) => {
  ...
  const active = isActive(platform);
  const accountMessage = getAccountMessage(platform);
    ...
    <Collapsible open={isOpen} onOpenChange={setIsOpen}>
      <CollapsibleTrigger className="w-full">
        <div className={`flex items-center justify-between p-4 rounded-xl border ${config.bgColor} ${config.borderColor} hover:shadow-sm transition-all cursor-pointer`}>
          {/* Left: icon + platform name + account message */}
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              ...
              <p className={`text-xs ${active ? 'text-muted-foreground' : 'text-muted-foreground italic'}`}>
                {accountMessage}
              </p>
            </div>
          </div>

          {/* Right: Active / Inactive badge + chevron */}
          <div className="flex items-center gap-3">
            {active ? (
              <span className="flex items-center gap-1 text-xs font-medium text-success bg-success/10 border border-success/20 px-2.5 py-1 rounded-full">
                <CheckCircle2 className="w-3.5 h-3.5" />
                Active
              </span>
            ) : (
              <span className="flex items-center gap-1 text-xs font-medium text-muted-foreground bg-muted border border-border px-2.5 py-1 rounded-full">
      ...
      <CollapsibleContent>
        <div className="mt-2 p-4 bg-muted/50 rounded-xl space-y-4">
          {active ? (
            <>
              {/* Activity metrics — posts & engagement only */}
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-background p-3 rounded-lg">
                  <div className="flex items-center gap-1.5 mb-1">
                    <FileText className="w-3.5 h-3.5 text-muted-foreground" />
                    <span className="text-xs text-muted-foreground">Posts / month</span>
                  </div>
            ...
            <div className="flex items-start gap-2 text-sm text-muted-foreground">
              <XCircle className="w-4 h-4 mt-0.5 flex-shrink-0 text-danger/60" />
              <p>This hotel does not appear to have a dedicated {config.label} account.</p>
            </div>
          )}
